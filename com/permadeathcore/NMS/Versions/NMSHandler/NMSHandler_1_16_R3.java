...
public class NMSHandler_1_16_R3 implements NMSHandler {
   ...
   public Entity spawnNMSEntity(String name, EntityType type, Location location, SpawnReason reason) {
      ...
      try {
         ...
         if (!name.toLowerCase().contains("bat") && !name.toLowerCase().contains("cod") && !name.toLowerCase().contains("salmon") && !name.toLowerCase().contains("squid") && !name.toLowerCase().contains("pufferfish") && !name.toLowerCase().contains("tropicalfish")) {
            nmsEntity = (net.minecraft.server.v1_16_R3.Entity)c.getConstructor(this.getNMSClass("EntityTypes"), this.getNMSClass("World")).newInstance(this.convertBukkitToNMS(type), nmsW);
   ...
   public Entity spawnNMSWither(Location location) {
      ...
      EntityWither wither = (EntityWither)EntityTypes.WITHER.spawnCreature((WorldServer)nmsW, compound, (IChatBaseComponent)null, (EntityHuman)null, new BlockPosition(location.getBlockX(), location.getBlockY(), location.getBlockZ()), EnumMobSpawn.EVENT, true, false);
