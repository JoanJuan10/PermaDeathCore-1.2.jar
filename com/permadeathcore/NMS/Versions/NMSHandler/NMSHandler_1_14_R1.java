...
public class NMSHandler_1_14_R1 implements NMSHandler {
   ...
   public Entity spawnNMSEntity(String name, EntityType type, Location location, SpawnReason reason) {
      ...
      try {
         ...
         if (!name.toLowerCase().contains("bat") && !name.toLowerCase().contains("cod") && !name.toLowerCase().contains("salmon") && !name.toLowerCase().contains("squid") && !name.toLowerCase().contains("pufferfish") && !name.toLowerCase().contains("tropicalfish")) {
            nmsEntity = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(this.getNMSClass("EntityTypes"), this.getNMSClass("World")).newInstance(this.convertBukkitToNMS(type), nmsW);
   ...
   public Entity spawnHostileMob(EntityType type, Location location, SpawnReason reason) {
      ...
      net.minecraft.server.v1_14_R1.Entity nms = null;
      String name;
      if (s.contains("_")) {
      ...
      try {
         ...
         if (!name.toLowerCase().contains("villager") && !name.toLowerCase().contains("wanderingtrader")) {
            if (!name.toLowerCase().contains("bat") && !name.toLowerCase().contains("cod") && !name.toLowerCase().contains("salmon") && !name.toLowerCase().contains("squid") && !name.toLowerCase().contains("pufferfish") && !name.toLowerCase().contains("tropicalfish")) {
               nms = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(Location.class, Main.getInstance().getNmsHandler().getNMSClass("EntityTypes")).newInstance(location, Main.getInstance().getNmsHandler().convertBukkitToNMS(type));
            } else if ((new Random()).nextInt(499) <= 10) {
               nms = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(Location.class, Main.getInstance().getNmsHandler().getNMSClass("EntityTypes")).newInstance(location, Main.getInstance().getNmsHandler().convertBukkitToNMS(type));
            }
         } else if (name.toLowerCase().contains("villager")) {
            nms = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(Location.class, Main.getInstance().getNmsHandler().getNMSClass("EntityTypes"), Villager.class).newInstance(location, Main.getInstance().getNmsHandler().convertBukkitToNMS(type), null);
         } else {
            nms = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(Location.class, Main.getInstance().getNmsHandler().getNMSClass("EntityTypes"), WanderingTrader.class).newInstance(location, Main.getInstance().getNmsHandler().convertBukkitToNMS(type), null);
         }
   ...
   public Entity spawnNMSCustomEntity(String name, EntityType type, Location location, SpawnReason reason) {
      ...
      try {
         ...
         nmsEntity = (net.minecraft.server.v1_14_R1.Entity)c.getConstructor(Location.class).newInstance(location);
   ...
   public Entity spawnCustomCreeper(Location location, SpawnReason reason, boolean is) {
      World nmsW = ((CraftWorld)location.getWorld()).getHandle();
