...
public class NMSHandler_1_15_R1 implements NMSHandler {
   ...
   public Entity spawnNMSEntity(String name, EntityType type, Location location, SpawnReason reason) {
      ...
      try {
         ...
         if (!name.toLowerCase().contains("bat") && !name.toLowerCase().contains("cod") && !name.toLowerCase().contains("salmon") && !name.toLowerCase().contains("squid") && !name.toLowerCase().contains("pufferfish") && !name.toLowerCase().contains("tropicalfish")) {
            nmsEntity = (net.minecraft.server.v1_15_R1.Entity)c.getConstructor(this.getNMSClass("EntityTypes"), this.getNMSClass("World")).newInstance(this.convertBukkitToNMS(type), nmsW);
   ...
   public Entity spawnCustomCreeper(Location location, SpawnReason reason, boolean isEnder) {
      World nmsW = ((CraftWorld)location.getWorld()).getHandle();
   ...
   public void addMushrooms() {
      ...
      try {
         BiomeMushrooms m = (BiomeMushrooms)BiomeMushrooms.class.newInstance();
         Field f = c.getDeclaredField("v");
         f.setAccessible(true);
         Map<EnumCreatureType, List<BiomeMeta>> v = (Map)f.get(m);
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.SPIDER, 100, 4, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.ZOMBIE, 95, 4, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.ZOMBIE_VILLAGER, 5, 1, 1));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.SKELETON, 100, 4, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.CREEPER, 100, 4, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.SLIME, 100, 4, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.ENDERMAN, 10, 1, 4));
         ((List)v.get(EnumCreatureType.MONSTER)).add(new BiomeMeta(EntityTypes.WITCH, 5, 1, 1));
         f.set(m, v);
