...
public class Cuboid implements Cloneable, ConfigurationSerializable, Iterable<Block> {
   ...
   public double getVolume() {
      return (this.getUpperX() - this.getLowerX() + 1.0D) * (this.getUpperY() - this.getLowerY() + 1.0D) * (this.getUpperZ() - this.getLowerZ() + 1.0D);
   }

   public World getWorld() {
      World world = Bukkit.getServer().getWorld(this.worldName);
      if (world == null) {
         throw new NullPointerException("El mundo '" + this.worldName + "' no ha sido cargado.");
      } else {
         return world;
      }
   }
